...
### Focus #2 (DONE): boss knowledge matched by encounterID + noise suppression
`resolveBossKey(encounterId, fallbackName)` (boss-knowledge.js) maps an encounter id → the canonical boss-map key; all three boss maps share one key per boss, so a single id lookup drives every access. `encounterId` added to all 9 `BOSS_KNOWLEDGE_META` entries; analyze.js/render.js/ai.js route through the resolver (falls back to name for unmapped bosses). This lights up boss knowledge in **local mode** (raw-log names differ from WCL) and is robust for WCL too — and it makes the stub name spellings irrelevant, so the earlier "rename Vaelgor &/Fallen-King/Belo'ren" task is **moot** (matching is by id, not spelling; no renames done). Noise the v2 parse exposed: `Shadow Word: Death` + `Spirit Link` (+ `Blessing of Dawn`) added to `selfInflictedAbilitiesGlobal` (globals.js) — they were false-flagging on every boss incl. live Chimaerus; `Fearsome Cry`/`Essence Bolt` added to Chimaerus NON_AVOIDABLE (group interrupt-failure, not individual). Verified in-browser: 3306→canonical key, 3179 (local "Fallen-King") → "Fallen King Salhadaar", etc.

### Focus #1 (DONE): 8 stub bosses filled from the v2 report
avoidableSpellIds + interruptTargetSpellIds + comprehensive NON_AVOIDABLE for all 8, classified from the v2 boss signals (per-spell dmgHits, distinct-player fraction, dodge/miss, **interrupted counts**, debuffs) cross-checked with the guide text. Method: avoidable = guide-says-dodge AND low raid-fraction (high-fraction = raid-wide → NON_AVOIDABLE; the dodge/parry signal flags *physical/tank* attacks, not raid mechanics, so it informed NON_AVOIDABLE not avoidable); interrupt targets = data-confirmed `interrupted > 0`. The v2 data revealed interrupt targets the guides never had: **Pitch Bulwark/Black Miasma (Imperator), Voidbolt (Vaelgor), Shadow Fracture (Salhadaar, 51×), Blinding Light (Lightblinded), Void Barrage (Crown), Light/Void Eruption (Belo'ren, ~200×), Safeguard (Midnight Falls, 131×)**. Per-boss notes: Belo'ren + Crown have NO avoidableSpellIds (Belo'ren mistakes are colour-assignment-gated → not in damage data, value is interrupt tracking; Crown is 1 Normal pull, low confidence). Vaelgor/Midnight Falls are Heroic-max (no Mythic mechanics); Lightblinded/Crown are Normal-only — revisit avoidables when Mythic logs exist. Interrupt-target damage names are in NON_AVOIDABLE so a missed cast's damage never shows as an individual avoidable hit.

**Local mode is now fully functional end-to-end** (fast + deep + boss knowledge) for any boss with WCL-verified ids, with no WarcraftLogs account. Still outstanding: **Rotmire** (add-or-not, Christian's call); live end-to-end test of local mode with a real log + Anthropic key (Christian); avoidable-ID refinement for the 4 bosses lacking Mythic logs.
