...
- **Spell IDs from `Trivial.txt` — investigated, NOT viable (2026-06-02):** only ONE clean `Spelldata` (id/name) block exists; remaining IDs are in "Affected By" modifier tables (~1,401 rows/409 names) that are talents/passives/auras, not abilities. Target rotational/defensive/interrupt abilities are absent or ambiguous (Metamorphosis→3 IDs, Immolation Aura→14). Rejected as a source — would introduce wrong IDs. The right path to close the spell-ID gap: harvest name→gameID from **live WCL data at analysis time** (the table already exposes ability `guid`; `abilityGuidByName` already captures it) and/or a focused live-Wowhead pass.
