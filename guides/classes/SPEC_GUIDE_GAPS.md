...
>
> **Coverage limit (important):** the report only contains spells the Patchwerk single-target sim *used* — damage/rotational abilities + simmed buffs. **Non-damaging defensives, interrupts, and utility are ABSENT** (Mind Freeze, Kick, Cloak of Shadows, Ice Block, Blur, Astral Shift, Dispersion, …). Those IDs come from the spec guides' Wowhead-confirmed values (e.g. Kick=1766, Mind Freeze=47528) and the `DEFENSIVE_SPELL_IDS` map — or a live-Wowhead/WCL pass.
>
> **UPDATE (2026-06-02): the defensive/interrupt gap now HAS a reference** — `simc-guides/interrupt-defensive-ids.json`, a live-researched (Wowhead + wowdb + warcraft.wiki.gg) map of every class's interrupt(s) and each spec's personal defensives with confirmed IDs (Mind Freeze 47528, Disrupt 183752, Skull Bash 106839, Quell 351338, Counter Shot 147362 / Muzzle 187707, Counterspell 2139, Spear Hand Strike 116705, Rebuke 96231, Silence 15487, Kick 1766, Wind Shear 254410, Spell Lock 19647 / Axe Toss 89766, Pummel 6552). Two values conflict with the functional `DEFENSIVE_SPELL_IDS` map (Alter Time 108978/342245, Die by the Sword 236385/118038) — flagged in that file's `_meta`, verify before reconciling. The reference is documentation only; the functional map is unchanged pending review.
